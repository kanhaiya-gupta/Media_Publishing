...
def load_user_article_interactions(client, limit=100000):
    ...
    try:
        ...
        SELECT 
            ...
            countIf(brand != '' AND brand IS NOT NULL) as sessions_with_brand
    ...
    # Try page_analytics first, then fall back to parsing JSON from session_metrics
    ...
    try:
        ...
        GROUP BY user_id, brand, unique_categories_count
        ORDER BY interaction_count DESC
        LIMIT {limit}
        """
        try:
            ...
            SELECT 
                user_id,
                concat('brand_', coalesce(brand, 'unknown'), '_cat_', toString(unique_categories_count)) as article_id,
                count() as interaction_count,
                sum(article_views) as total_views,
                sum(article_clicks) as total_clicks,
                avg(session_duration_sec) as avg_time_spent,
    ...
    if len(results) > 0:
        ...
        if len(results[0]) == 7:
            df = pd.DataFrame(results, columns=[
                'user_id', 'article_id', 'interaction_count',
                'total_views', 'total_clicks', 'avg_time_spent', 'last_interaction'
            ])
        else:
            df = pd.DataFrame(results, columns=[
                'user_id', 'article_id', 'interaction_count',
                'total_views', 'total_clicks', 'last_interaction'
            ])
...
def create_content_features(client):
    ...
    WHERE (article_views > 0 OR article_clicks > 0)
    AND brand != '' AND brand IS NOT NULL
    GROUP BY brand, unique_categories_count
    """
    
    results = client.execute(query)
    
    df = pd.DataFrame(results, columns=[
        'article_id', 'brand', 'category_count', 'unique_readers',
        'total_views', 'total_clicks', 'avg_time_spent', 'total_interactions'
    ])
    ...
    numerical_features = ['category_count', 'unique_readers', 'total_views', 'total_clicks', 'avg_time_spent', 'total_interactions']
...
def evaluate_recommendations(user_item_matrix, model, W, H, content_features, n_test_users=100):
    ...
    # Sample test users
    test_users = np.random.choice(user_item_matrix.index, size=min(n_test_users, len(user_item_matrix)), replace=False)
