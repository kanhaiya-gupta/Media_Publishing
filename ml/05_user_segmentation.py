...
def find_optimal_clusters(X, max_clusters=10):
    ...
    for k in k_range:
        ...
        print(
            f"  k={k}: Inertia={kmeans.inertia_:.2f}, Silhouette={silhouette_scores[-1]:.4f}, DB={davies_bouldin_scores[-1]:.4f}"
        )
...
def analyze_segments(df, labels, feature_list):
    ...
    for cluster_id in range(n_clusters):
        ...
        profile = {
            ...
            "avg_engagement_score": cluster_data.get(
                "avg_engagement_score", cluster_data.get("engagement_per_session", 0)
            ).mean(),
            ...
            "subscription_tier": (
                ...
                if "current_subscription_tier" in cluster_data.columns
                else "N/A"
            ),
        }
...
def visualize_segments(df, labels, pca, kmeans, scaler, clustering_features):
    ...
    segment_engagement = (
        df.groupby("segment")["avg_engagement_score"].mean()
        if "avg_engagement_score" in df.columns
        else df.groupby("segment")["engagement_per_session"].mean()
    )
