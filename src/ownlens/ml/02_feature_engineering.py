...
def extract_user_features(client, limit=10000):
    ...
    count_query = "SELECT count() as total_sessions FROM customer_sessions"
    try:
        count_result = client.execute(count_query)
        total_sessions = count_result[0][0] if count_result else 0
        print(f"  Found {total_sessions:,} total sessions in customer_sessions")

        if total_sessions == 0:
            print("⚠️  No data in customer_sessions table")
            return pd.DataFrame(columns=columns)

        # Check unique users count
        user_count_query = "SELECT uniqExact(user_id) as unique_users FROM customer_sessions"
    ...
    SELECT 
        ...
        -- Engagement Score (use pre-calculated if available, otherwise calculate)
        avg(COALESCE(sm.engagement_score, 
            (sm.article_views * 2 + 
             sm.article_clicks * 3 + 
             sm.video_plays * 5 + 
        ...
        -- Content Features (using brand_id - UUID)
        argMax(sm.brand_id, sm.session_start) as preferred_brand_id,
        
        -- Geographic Features
        argMax(sm.country_code, sm.session_start) as primary_country_code,
        argMax(sm.city_id, sm.session_start) as primary_city_id,
        
        -- Device Features (using IDs - UUIDs)
        argMax(sm.device_type_id, sm.session_start) as primary_device_type_id,
        argMax(sm.os_id, sm.session_start) as primary_os_id,
        argMax(sm.browser_id, sm.session_start) as primary_browser_id,
...
def create_derived_features(df):
    ...
    df["brand_loyalty_score"] = df[
        [
            "brand_bild_ratio",
            "brand_welt_ratio",
            "brand_bi_ratio",
            "brand_politico_ratio",
            "brand_sport_bild_ratio",
        ]
    ].max(axis=1)
