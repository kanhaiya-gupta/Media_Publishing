...
def get_data_summary(client):
    ...
    # Total sessions
    total_sessions = client.execute("SELECT count() FROM customer_sessions")[0][0]
    ...
    # Unique users
    unique_users = client.execute("SELECT count(DISTINCT user_id) FROM customer_sessions")[0][0]
    print(f"Unique Users: {unique_users:,}")

    # Brands distribution (using brand_id - may need join with brands table for names)
    print("\nBrands Distribution:")
    brands = client.execute(
        """
        SELECT brand_id, count() as sessions
        FROM customer_sessions
        GROUP BY brand_id
        ORDER BY sessions DESC
    """
    )
    for brand_id, count in brands:
        print(f"  {brand_id}: {count:,} sessions ({count/total_sessions*100:.1f}%)")
    ...
    for country_code, count in countries:
        print(f"  {country_code}: {count:,} sessions")

    # Device types (using device_type_id - may need join with device_types table for names)
    print("\nDevice Types Distribution:")
    devices = client.execute(
        """
        SELECT device_type_id, count() as sessions
        FROM customer_sessions
...
def get_user_level_features(client, sample_size=10000):
    ...
    SELECT 
        ...
        argMax(country_code, session_start) as primary_country_code,
        argMax(device_type_id, session_start) as primary_device_type_id,
        argMax(brand_id, session_start) as preferred_brand_id,
