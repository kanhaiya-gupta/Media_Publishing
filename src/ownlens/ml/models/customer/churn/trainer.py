"""
OwnLens - ML Module: Churn Trainer
...
from ....features.customer.churn_features import ChurnFeatureEngineer
...
class ChurnTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        evaluator: Optional[ChurnEvaluator] = None,
        use_feature_engineers: bool = True
    ):
        """
        Initialize churn trainer.
        
        Args:
            model: Churn model instance (if None, creates default)
            evaluator: Churn evaluator instance (if None, creates default)
        ...
        super().__init__(model, feature_engineer, evaluator)
        self.logger.info("Initialized churn trainer")
    ...
    def prepare_features(
        ...
        **kwargs
        ...
        Creates churn label based on days_since_last_session.
        ...
        if 'days_since_last_session' in data.columns:
            y = (data['days_since_last_session'] >= 30).astype(int)
        elif 'churned_user' in data.columns:
            y = data['churned_user']
        else:
            raise ValueError("Cannot create churn label: missing 'days_since_last_session' or 'churned_user' column")
        
        self.logger.info(f"Churn distribution: {(y == 0).sum()} not churned, {(y == 1).sum()} churned")
        ...
        if len(unique_classes) < 2:
            ...
            raise ValueError(f"Cannot train churn model: only one class found ({unique_classes[0]}). Need at least 2 classes.")
    ...
    def train(
        ...
        **kwargs
        ...
        # Evaluate model
        if test_size > 0 and self.evaluator:
