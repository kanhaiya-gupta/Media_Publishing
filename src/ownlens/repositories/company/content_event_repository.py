"""
OwnLens - Company Domain: Content Event Repository

Repository for company_content_events table.
"""
...
from ...models.company.content_event import ContentEvent, ContentEventCreate, ContentEventUpdate
...
class ContentEventRepository(BaseRepository[ContentEvent]):
    ...
    def __init__(self, connection_manager):
        ...
        super().__init__(connection_manager, table_name="company_content_events")
        self.content_events_table = "company_content_events"
    ...
    async def create_content_event(self, event_data: Union[ContentEventCreate, Dict[str, Any]]) -> Optional[ContentEvent]:
        ...
        try:
            ...
            result = await self.create(data, table_name=self.content_events_table)
            if result:
    ...
    async def get_content_events_by_content(self, content_id: UUID, limit: int = 100, offset: int = 0) -> List[ContentEvent]:
        ...
        try:
            ...
            logger.error(f"Error getting content events by content {content_id}: {e}")
            return []

    async def get_content_events_by_employee(self, employee_id: UUID, limit: int = 100, offset: int = 0) -> List[ContentEvent]:
        """Get all events by an employee."""
        try:
            results = await self.get_all(
            ...
            logger.error(f"Error getting content events by employee {employee_id}: {e}")
            return []

    async def get_content_events_by_type(self, event_type: str, company_id: Optional[UUID] = None, limit: int = 100, offset: int = 0) -> List[ContentEvent]:
    ...
    async def get_content_events_by_date_range(self, start_date: date, end_date: date, company_id: Optional[UUID] = None) -> List[ContentEvent]:
        ...
        try:
            ...
            if company_id:
                query = query.replace("WHERE", f"WHERE company_id = $1 AND")
                params = [company_id] + list(params) if params else [company_id]
