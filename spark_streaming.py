...
from pyspark.sql.functions import (
    from_json, col, min as spark_min, max as spark_max, expr, count, when,
    collect_list, struct, sum as spark_sum, to_json, from_unixtime, lit,
    first, avg as spark_avg
)
...
spark = SparkSession.builder \
    .appName("MediaPublishingRealTimeAnalytics") \
...
df_parsed = df.selectExpr("CAST(value AS STRING) as json_str") \
    ...
    .select("data.*") \
    .filter(col("event_type") != "session_end")  # Filter out session_end for session tracking
...
session_metrics = df_parsed.groupBy("session_id", "user_id") \
    .agg(
...
session_metrics_transformed = session_metrics \
    .withColumn("session_start_dt", from_unixtime(col("session_start"))) \
...
def write_to_storage_and_analytics(batch_df, batch_id):
    ...
    try:
        ...
        batch_df_with_id.select(
            "session_id", "user_id", "brand", "country", "device_type",
            "subscription_tier", "session_start", "session_end", "session_duration_sec",
            "total_events", "article_views", "pages_visited_count", "batch_id"
...
query = session_metrics_transformed.writeStream \
    .outputMode("complete") \
